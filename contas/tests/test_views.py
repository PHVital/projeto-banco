...
class TestRegistroClienteView:
    ...
    def test_registrar_cliente_cpf_duplicado(
        self, api_client, cliente_registrado_para_views, usuario_teste_data
    ):
        url = reverse("registrar_cliente")
    ...
    def test_registrar_cliente_campo_obrigatorio_faltando(
        self, api_client, cliente_registrado_para_views, usuario_teste_data
    ):
        url = reverse("registrar_cliente")
...
class TestLoginClienteView:
    def test_login_sucesso(
        ...
        assert Token.objects.filter(
            key=response.data["token"], user=cliente_registrado_para_views
        ).exists()

    def test_login_credenciais_invalidas_senha(
        self, api_client, cliente_registrado_para_views
    ):
        url = reverse("autenticar_cliente")
        payload = {
            "cpf": cliente_registrado_para_views.cpf,
    ...
    def test_login_usuario_nao_existe(self, api_client):
        url = reverse("autenticar_cliente")
        payload = {"cpf": "cpf_inexistente_000", "password": "qualquersenha"}
...
class TestOperacoesContaView:
    def test_deposito_sucesso(self, cliente_autenticado_com_conta):
        ...
        assert (
            Decimal(response.data["conta"]["saldo"]) == saldo_anterior + valor_deposito
        )

        conta_obj.refresh_from_db()
        assert conta_obj.saldo == saldo_anterior + valor_deposito
        assert Transacao.objects.filter(
            conta=conta_obj, tipo="D", valor=valor_deposito
        ).exists()
