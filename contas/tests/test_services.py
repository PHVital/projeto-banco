...
def test_depositar_valor_sucesso(conta_generica):
    ...
    conta_atualizada = depositar_valor(
        numero_conta=conta_mock.numero_conta, valor=valor_a_depositar
    )
...
def test_depositar_valor_negativo_ou_zero(conta_generica):
    ...
    with pytest.raises(
        ValueError, match="O valor do depósito deve ser maior que zero."
    ):
...
def test_sacar_valor_sucesso(conta_generica):
    ...
    conta_atualizada = sacar_valor(
        numero_conta=conta_mock.numero_conta, valor=valor_a_sacar
    )
...
@pytest.mark.django_db
def test_transferir_valor_saldo_insuficiente_origem(
    conta_origem_com_saldo, conta_destino_com_saldo
):
...
def test_transferir_para_mesma_conta(conta_origem_com_saldo):
    with pytest.raises(
        ValueError, match="Conta de origem e destino não podem ser iguais."
    ):
        transferir_valor(
            conta_origem_numero=conta_origem_com_saldo.numero_conta,
...
@pytest.mark.django_db
def test_transferir_valor_negativo_ou_zero(
    conta_origem_com_saldo, conta_destino_com_saldo
):
...
def test_transferir_conta_origem_nao_encontrada(conta_destino_com_saldo):
    with pytest.raises(
        ValueError, match="Conta de origem NUMERO_FALSO não encontrada."
    ):
        transferir_valor(
