...
from contas.services.cliente_services import criar_cliente_e_conta, depositar_valor, sacar_valor
...
def registrar_cliente(request):
    ...
    if serializer.is_valid():
        try:
            ...
            return Response({'erro': 'Ocorreu um erro ao criar o cliente.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def deposito(request):
    ...
    if serializer.is_valid():
        ...
        try:
            conta_bancaria = get_object_or_404(ContaBancaria, numero_conta=numero_conta_req)
            ...
            return Response({'erro': f'Conta {numero_conta_req} não encontrada.'}, status=status.HTTP_404_NOT_FOUND)
        except ValueError as e:
            return Response({'erro': str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({'erro': 'Ocorreu um erro ao processar o depósito'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def saque(request):
    ...
    if serializer.is_valid():
        ...
        try:
            conta_bancaria = get_object_or_404(ContaBancaria, numero_conta=numero_conta_req)
            if conta_bancaria.cliente != request.user:
                return Response({'erro': 'Você não tem permissão para sacar nesta conta.'}, status=status.HTTP_403_FORBIDDEN)
            ...
            return Response({'erro': 'Ocorreu um erro ao processar o saque'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def consultar_saldo(request):
    try:
        ...
        return Response({'erro': 'Nenhuma conta bancária encontrada para este cliente.'}, status=status.HTTP_404_NOT_FOUND)
    except ContaBancaria.MultipleObjectsReturned:
        return Response({'erro': 'Múltiplas contas encontradas. Especifique a conta.'}, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
        return Response({'erro': 'Ocorreu um erro ao consultar o saldo.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def extrato_transacoes(request):
    try:
        ...
        return Response({'erro': 'Ocorreu um erro ao buscar o extrato.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def transferencia(request):
    ...
    if serializer.is_valid():
        ...
        try:
            conta_origem_obj = get_object_or_404(ContaBancaria, numero_conta=conta_origem_num)
            ...
            return Response({'erro': 'Ocorreu um erro ao processar a transferência.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)