...
class ClienteCreateSerializer(serializers.ModelSerializer):
    ...
    data_nascimento = serializers.DateField(required=False, allow_null=True, input_formats=["%Y-%m-%d"])
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if cpf and password:
            ...
            if not user:
                raise serializers.ValidationError('Credenciais inválidas. Verifique CPF e senha.', code='authorization')
        else:
...
class TransferenciaSerializer(serializers.Serializer):
    conta_origem = serializers.CharField(max_length=12, required=True, help_text='Número da sua conta de origem')
    conta_destino = serializers.CharField(max_length=12, required=True, help_text='Número da conta de destino')
    valor = serializers.DecimalField(max_digits=10, decimal_places=2)

    def validate_valor(self, value):
        if value <= Decimal('0.00'):
            raise serializers.ValidationError('O valor da transferência deve ser positivo.')
        return value
